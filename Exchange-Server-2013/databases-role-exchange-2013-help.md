---
title: 'Ruolo del database: Exchange 2013 Help'
TOCTitle: Ruolo del database
ms:assetid: 317d9945-668b-448e-963f-f7127b9b3d79
ms:mtpsurl: https://technet.microsoft.com/it-it/library/Dd876868(v=EXCHG.150)
ms:contentKeyID: 50480281
ms.date: 05/22/2018
...
Il ruolo di gestione `Databases` consente agli amministratori di creare, gestire, montare e smontare i database delle cassette postali su singoli server.
...
Per utilizzare il cmdlet **Move-DatabasePath**, incluso con questo ruolo, è necessario che il database che si desidera configurare si trovi nell'ambito dei database e che il database risieda in un server che si trova nell'ambito dei server.

Inoltre, per utilizzare il cmdlet **Remove-MailboxDatabase** incluso con questo ruolo, è necessario che il database che si desidera rimuovere si trovi nell'ambito dei database o che il database risieda su un un server che si trova nell'ambito dei server. Ciò significa che l'utente controlla chi può rimuovere i database delle cassette postali utilizzando gli ambiti di database o server.
