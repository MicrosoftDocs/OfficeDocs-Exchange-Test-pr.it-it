---
title: 'Ruolo destinatari di posta elettronica: Exchange 2013 Help'
TOCTitle: Ruolo destinatari di posta elettronica
ms:assetid: 7cf1f1c7-4f32-4446-ac04-99a7f9df2059
ms:mtpsurl: https://technet.microsoft.com/it-it/library/Dd876911(v=EXCHG.150)
ms:contentKeyID: 50481015
ms.date: 05/22/2018
...
Il ruolo di gestione `Mail Recipients` consente agli amministratori di gestire contatti, utenti di posta elettronica e cassette postali esistenti in un'organizzazione. Questo ruolo non è in grado di creare questi destinatari. Utilizzare il ruolo `Mail Recipient Creation` per crearli.

Questo tipo di ruolo non consente all'utente di gestire le cartelle pubbliche abilitate alla posta o i gruppi di distribuzione. Per gestire questi oggetti, utilizzare i seguenti ruoli:
...
Se l'organizzazione dispone di un modello di divisione delle autorizzazioni per cui la gestione e la creazione del destinatario vengono eseguite da gruppi diversi, assegnare il ruolo `Mail Recipient Creation` al gruppo che esegue la creazione e il ruolo `Mail Recipients` al gruppo che esegue la gestione. Per ulteriori informazioni, vedere i seguenti argomenti:
...
Oltre agli ambiti dei destinatari, i cmdlet **Connect-Mailbox** e **Enable-Mailbox**, inclusi con questo ruolo, vengono indicati come ambiti tramite l'uso degli ambiti di configurazione del database. Gli ambiti di configurazione del database controllano su quali database i cmdlet possono creare nuove cassette postali. Il database in cui si desidera creare una cassetta postale deve trovarsi all'interno dell'ambito di database. Tale condizione viene applicata quando si specifica un database utilizzando il parametro *Database* sul cmdlet o se si consente la distribuzione automatica delle cassette postali per la selezione del database. Per ulteriori informazioni, vedere [Comprensione degli ambiti di gestione dei ruoli](understanding-management-role-scopes-exchange-2013-help.md).
