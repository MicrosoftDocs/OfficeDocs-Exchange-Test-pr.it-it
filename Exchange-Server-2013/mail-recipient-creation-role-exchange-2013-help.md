---
title: 'Ruolo di creazione dei destinatari di posta: Exchange 2013 Help'
TOCTitle: Ruolo di creazione dei destinatari di posta
ms:assetid: 81081845-9ebf-4888-8f9f-fe7cf2704486
ms:mtpsurl: https://technet.microsoft.com/it-it/library/Dd876915(v=EXCHG.150)
ms:contentKeyID: 50481056
ms.date: 05/22/2018
...
Questo ruolo non consente di abilitare le cartelle pubbliche all'utilizzo della posta elettronica. Per abilitare le cartelle pubbliche all'utilizzo della posta elettronica, è necessario utilizzare il ruolo `Mail Enabled Public Folders`. Per ulteriori informazioni, vedere [Ruolo di cartelle pubbliche abilitate alla posta](mail-enabled-public-folders-role-exchange-2013-help.md).

Se l'organizzazione mantiene un modello di divisione delle autorizzazioni del controllo di accesso basato sui ruoli (RBAC), in cui la creazione dei destinatari viene eseguita da un gruppo diverso e non dal gruppo che esegue la gestione dei destinatari, assegnare il ruolo `Mail Recipient Creation` al gruppo di ruoli di gestione che esegue la creazione dei destinatari e il ruolo `Mail Recipients` al gruppo di ruoli che esegue la gestione dei destinatari.

Se nell'organizzazione la divisione delle autorizzazioni di Active Directory è abilitata, tutte le assegnazioni dei ruoli di gestione non di delega per il ruolo di gestione corrente sono state rimosse. Se la divisione delle autorizzazioni di Active Directory è abilitata, solo gli amministratori di Active Directory che utilizzano gli strumenti di gestione di Active Directory possono creare nuove entità di sicurezza quali, ad esempio, utenti e gruppi di protezione.

Per ulteriori informazioni sul controllo dell'accesso basato sui ruoli (RBAC) e sulla divisione delle autorizzazioni di Active Directory, vedere [Informazioni sulle autorizzazioni diviso](understanding-split-permissions-exchange-2013-help.md).
...
Oltre agli ambiti dei destinatari, anche l'ambito del cmdlet **New-Mailbox**, incluso in questo ruolo, è stato definito utilizzando gli ambiti di configurazione del database. Gli ambiti di configurazione del database designano i database in cui creare nuove cassette postali utilizzando i cmdlet. Il database in cui si desidera creare una cassetta postale deve trovarsi nell'ambito del database. Questa condizione si verifica quando si specifica un database utilizzando il parametro *Database* nel cmdlet **New-Mailbox** o se si seleziona il database mediante la distribuzione automatica delle cassette postali. Per ulteriori informazioni, vedere [Comprensione degli ambiti di gestione dei ruoli](understanding-management-role-scopes-exchange-2013-help.md).
